pipeline {
    ...
    parameters {
        ...
        string(name: 'DOCKER_IMAGE_TAG', defaultValue: 'latest', description: 'Docker image tag to test in the functional pipeline.')
        ...
        booleanParam(name: 'PULL_IMAGE_BEFORE_TEST', defaultValue: true, description: 'Pull the image tag from a registry before docker compose up.')
        booleanParam(name: 'KEEP_STACK_RUNNING', defaultValue: false, description: 'Leave the docker compose stack running after a successful functional test run.')
    }
    ...
    stages {
        ...
        stage('Pull Docker Image') {
            when {
                expression { params.PULL_IMAGE_BEFORE_TEST && env.RESOLVED_IMAGE_NAME.contains('/') }
            }
            steps {
                powershell '''
        ...
        always {
            script {
                if (fileExists(env.COMPOSE_ENV_FILE)) {
                    ...
                    if (!params.KEEP_STACK_RUNNING || currentBuild.currentResult != 'SUCCESS') {
